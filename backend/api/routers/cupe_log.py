...
@router.get('/next-{entity_type}')
def get_next_cupe(entity_type: str, user: dict = Depends(get_current_user)):
    require_superadmin(user)
    models = {'client': (Client, 'CLI'), 'collaborator': (Collaborator, 'ELO')}
    if entity_type not in models:
        raise HTTPException(status_code=400, detail='Tipo de entidad inválido')
    model, prefix = models[entity_type]
    for _ in range(20):
        candidate = f'{prefix}-{secrets.randbelow(100_000_000):08d}'
        if not model.objects.filter(cupe=candidate).exists():
            return {'next_cupe': candidate}
    raise HTTPException(status_code=503, detail='No se pudo generar un CUPE disponible')
...
def change_cupe(
    ...
    # Solo Superadmin puede autorizar cambios de CUPE
    require_superadmin(user)
    prefix = {'client': 'CLI', 'collaborator': 'ELO'}.get(data.entity_type)
    if not prefix:
        raise HTTPException(status_code=400, detail='Tipo de entidad inválido')
    if not re.fullmatch(rf'{prefix}-\d{{8}}', data.new_cupe):
        raise HTTPException(status_code=400, detail='Formato de CUPE inválido')
    if data.reason not in dict(CupeLog.REASON_CHOICES):
        raise HTTPException(status_code=400, detail='Motivo de cambio inválido')
    if data.reason == 'otro' and not (data.observations or '').strip():
    ...
    with transaction.atomic():
        ...
        if data.entity_type == 'client':
            # Buscar el cliente
            try:
                entity = Client.objects.select_for_update().get(id=data.entity_id)
            ...
            try:
                entity = Collaborator.objects.select_for_update().get(id=data.entity_id)
            except Collaborator.DoesNotExist:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
        ...
        if old_cupe == data.new_cupe:
            raise HTTPException(status_code=400, detail='El nuevo CUPE debe ser diferente')
